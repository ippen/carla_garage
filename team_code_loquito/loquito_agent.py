...
from utils import convert_numpy_image_to_tensor, PIDController
...
class LoquitoAgent(AutonomousAgent):
    ...
    def save(self, input_data, idx, timestamp, target_waypoint, target_waypoint_in_cs0, throttle, steering, brake,
        ...
        print(f"Saving data for timestamp {timestamp:.3f} with idx {idx}")
        data = {
            "idx": int(idx),
        ...
        print(f"Saved json data for timestamp {timestamp:.3f} with idx {idx}")
    ...
    def _compute_control(self, waypoints, stopping, steering, current_velocity):
        ...
        if stopping >= self.stopping_threshold:
            ...
            if accel_cmd > self.deadband:
                throttle = np.clip(self.min_actuation + accel_cmd, 0, 1.0)
                brake = 0.0
            elif accel_cmd < -self.deadband:
                throttle = 0.0
                brake = np.clip(self.min_actuation - accel_cmd, 0, 1.0)
            else:
                throttle = 0.0
                brake = 0.0
